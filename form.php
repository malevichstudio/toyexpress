...
use app\modules\video\widgets\BackendVideoWidget;
...
if (($model->parent_id > 0) && (null !== $parent = Category::findById($model->parent_id, null, null))) {
    ...
    <div class="row">
        <article class="<?= Helper::getBackendGridClass('shop', ShopModule::BACKEND_CATEGORY_GRID, 1) ?>">
            ...
            if($model->isNewRecord === false) {
                echo $form->field($model, 'parent_id')->dropDownList(
                    ArrayHelper::merge(
                        [0 => Yii::t('app', 'Root')],
                        ArrayHelper::map(
                            Category::find()->where('id != :id', ['id' => $model->id])->all(),
                            'id',
                            'name'
                        )
                    )
            ...
            <?= $form->field($model, 'slider_id')->dropDownList(
                \app\components\Helper::getModelMap(Slider::className(), 'id', 'name'), ['prompt' => Yii::t('app', 'No')]) ?>

            <?=$form->field($model, 'show_filter')->widget(\kartik\switchinput\SwitchInput::className())?>
            <?=$form->field($model, 'show_sub_categories')->widget(\kartik\switchinput\SwitchInput::className())?>
            <?=$form->field($model, 'google_category')?>
            ...
            <div id="videos">
                <input type="hidden" name="videos" id="video_params">
                <?php
                BackendWidget::begin(
                    [
                ...
                <?= BackendVideoWidget::widget(['model' => $model]) ?>
                <?php BackendWidget::end(); ?>
            </div>
        </article>
